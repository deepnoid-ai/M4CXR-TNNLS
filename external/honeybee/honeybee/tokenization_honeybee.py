...
from ..pipeline.data_utils.constants import AI, HUMAN, IGNORE_INDEX, MEDIA_TOKENS
    ...
    def encode_prompt(self, prompt: str, max_length: int | None, no_eos=False):
        ...
        if prompt.startswith("[NO_PROMPT] <image> "):
            ...
            enc_chunk += (
                ...
                * self.media_lengths["<image>"]  # media tokens
            ...
            pattern = "|".join(
                map(re.escape, list(self.media_tokens.keys()) + [_AI, _HUMAN])
            )
...
def build_honeybee_tokenizer(
    pretrained_tokenizer_name_or_path: str, num_visual_tokens: int
):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        pretrained_tokenizer_name_or_path, use_fast=False
    )
