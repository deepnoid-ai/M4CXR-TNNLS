...
# local folder import
import preprocess.mimic_cxr.section_parser as sp

parser = argparse.ArgumentParser()
parser.add_argument(
    "--reports_path",
    default="datasets/MIMIC-CXR/text_files",
    # required=True,
    help=("Path to file with radiology reports," " e.g. /data/mimic-cxr/files"),
)
parser.add_argument(
    "--output_path",
    default="datasets/MIMIC-CXR/preprocess/v3.0/sections",
    # required=True,
    help="Path to output CSV files.",
)
parser.add_argument(
    "--no_split", action="store_false", help="Do not output batched CSV files."
)


def list_rindex(list_input, s):
    """Helper function: *last* matching element in a list"""
    return len(list_input) - list_input[-1::-1].index(s) - 1
...
def main(args):
    ...
    if len(patient_studies) > 0:
        ...
        if args.no_split:
            ...
            with open(output_path / "mimic_cxr_sections.csv", "w") as fp:
                csvwriter = csv.writer(fp)
