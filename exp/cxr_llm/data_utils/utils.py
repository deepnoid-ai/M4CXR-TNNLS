...
def get_cosine_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        ...
        cos_mult = max(
            0, 0.5 * (1.0 + math.cos(math.pi * float(num_cycles) * 2.0 * progress))
        )
...
def merge_consecutive_tokens(tensor, token=-1):
    # Compute the difference between consecutive elements
    diff = torch.diff(tensor, prepend=tensor[:1])

    # Find the indices where the difference is 0 and the original tensor is -1
    mask = (diff != 0) | (tensor != token)

    # Use the mask to get the desired elements
    result = tensor[mask]

    return result


def chexbert_text_cleansing(text):
    ...
    # Remove leading and trailing whitespace again after replacements
    text = text.strip()
    return text


# copy from PromptMRG/dataset/utils.py
# modified
# (1) remove blank before . : " . " => ". "
# (2) remove lower
# (3) remove multiple blank
