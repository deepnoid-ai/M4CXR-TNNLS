...
from exp.cxr_llm.data_utils.templates import (
    GEMMA_ASSISTANT,
    GEMMA_CHAT_TEMPLATE,
    GEMMA_USER,
    IGNORE_INDEX,
...
from external.honeybee.utils.logging import get_logger
...
# copy from Honeybee and modified
class MLLMTokenizerMixin:
    def mllm_setup(self, num_visual_tokens: int, chat_template: str):
        if self.pad_token is None:
            logger.warning(
                f"Tokenizer {self.__class__} has no pad_token. Use unk_token instead."
            )
        ...
        if chat_template == "mistral":
            ...
            self.user_indicator = GEMMA_USER
            self.assistant_indicator = GEMMA_ASSISTANT
        elif chat_template == "vicuna":
            self.chat_template = VICUNA_CHAT_TEMPLATE
            self.user_indicator = VICUNA_USER
            self.assistant_indicator = VICUNA_ASSISTANT
        else:
